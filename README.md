...
> GitHub Actions 的 `schedule` 本身**不保证准点**：高峰期可能延迟 10~30 分钟甚至更久，
> 极端时整轮被丢弃。因此本项目采用「**高频轮询 + 时间闸门 + 当天去重**」机制，
> 把实际误差压到约 ±5 分钟，并大幅降低漏发概率。
...
1. workflow 在**北京时间 10:00–13:55 每 5 分钟轮询一次**（`*/5 10-13 * * *`，`timezone: Asia/Shanghai`）。
2. 每次轮询先用一个很便宜的「时间闸门」判断：没到 `SEND_AT` 就直接结束，**不安装依赖、不消耗时间**。
3. 到了 `SEND_AT`（默认 `12:05`）之后，**当天第一次**命中且尚未发送的轮询才会真正安装依赖并发送。
4. 发送完成后用 GitHub 缓存写入「今天已发送」标记，当天后续轮询直接跳过，避免重复刷屏。

修改发送时间：在仓库 `Settings → Secrets and variables → Actions → Variables` 新增变量 `DOUYIN_SEND_AT`，值为 `HH:MM`（北京时间），例如 `09:30`。若同时修改，注意把 cron 的轮询窗口 `10-13` 也调整到覆盖目标时间并留出至少 1 小时兜底。

手动运行（`Run workflow`）会**绕过时间闸门**，随时可用 `dry_run` 测试。

> 说明：纯 GitHub 方案仍可能比 `SEND_AT` 晚几分钟（轮询间隔 + GitHub 自身延迟），
> 极端情况下若整个轮询窗口都被丢弃仍会漏发。若需要**秒级准点**，请改用自托管的
> `deploy/systemd/` 定时器或云函数触发（见下方「定时调度」说明）。
